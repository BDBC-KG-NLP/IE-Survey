...
| MAE       |        |        |       | Multimodal Attribute Extraction                              |
...
- 方法：
  ...
  ![QA模型结构](.\image\attribute\aveqa.png )
...
- 论文：《Multimodal Joint Attribute Prediction and Value Extraction for E-commerce Product》
  ...
  ![QA模型结构](.\image\attribute\multimodel.png )
...
### <span id="head25">3.4 可拓展方向</span>

- 少样本零样本研究：在其他nlp研究中，少样本和零样本学习得到了大规模的研究，但是在属性抽取中，少样本和零样本并没有作为一个主要的研究方向进行探索，如prototype network等没有在该领域得到验证。
- 抽取短语或句子：目前数据集处理的属性值通常为一个词语，而现实世界中，经常会出现属性是一大段描述性文字的情况，当前模型效果未知。
- 一个属性名对应多个属性值：一个实体的某个属性通常并不是一个属性值，如人的代表作可以不止一个，产品的制作材料也可以是多种材料混合。
- 篇章级属性抽取：当前工作集中于处理单一句子中的属性名和属性值，当属性数据分布在不同句子中时，模型无法抽取。因此需要考虑篇章级的属性抽取模型。
